...
router.post('/fermentation/projects', async (req, res) => {
  const result = await database.saveNewProject(req.body)
    .then(response => {
      if (response.status) {
        res.status(201).send({status: response.status, addedProject: response.addedProject});
      } else {
...
router.patch('/fermentation/projects', async (req, res) => {
  const result = await database.updateStatus(req.body)
    .then(response => {
      if (response === 1) {
        res.status(200);
      } else {
...
router.patch('/fermentation/projects/update', async (req, res) => {
  const result = await database.updateProject(req.body.id, req.body.project)
    .then(response => {
      if (response.status) {
...
router.delete('/fermentation/projects', async (req, res) =>{
  const result = await database.deleteProject(req.body)
    .then(response => {
      if (response === 1) {
