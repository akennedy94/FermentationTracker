...
const activeDB = new nedb({filename: 'fermentationProjects.db', autoload: true});
...
exports.updateStatus = async (idToUpdate) => {
    return new Promise((resolve, reject) => {
        activeDB.update({_id: idToUpdate.id}, {$set: {active: false}}, {}, (err, updatedDoc) => {
            if (err) reject(err);
...
exports.updateProject = async (idToUpdate, updatedProject) => {
    return new Promise((resolve, reject) => {
        activeDB.update({_id: idToUpdate}, 
            {$set: 
                {   
            ...
            if (err) reject(err);
            if (updatedDoc) resolve({status: true, updatedDoc: updatedDoc, id: idToUpdate});
        })
    }).catch(error=>console.log(error))
}
